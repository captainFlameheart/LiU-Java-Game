...
public class CameraDrawer implements Drawer
{
    ...
    private CameraDrawer(final DrawRegion camera, final Color color, final BasicStroke stroke) {
	this.camera = camera;
	this.color = color;
	this.stroke = stroke;
    }

    public static CameraDrawer createDashed(final DrawRegion camera, final Color color, final float strokeWidth) {
	final float[] dash = {1};
